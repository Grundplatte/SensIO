...
bool LPS25H::isEnabled() {
    ...
    _hal->read(LPS25H_I2C_ADDR0, LPS25H_CTRL_REG_1, 1, &data);
...
int LPS25H::enable() {
    ...
    _hal->write(LPS25H_I2C_ADDR0, LPS25H_CTRL_REG_1, 1, &data);
...
std::vector<int> LPS25H::getUnusedRegisters() {
    ...
    if(data & 0x08){
        ...
        _hal->read(LPS25H_I2C_ADDR0, LPS25H_CTRL_REG_3, 1, &data);
        if(data & 0x03){
            // Threshold in use
            return output;
        }
...
std::vector<int> LPS25H::getResultRegisters() {
    ...
    resultRegisters.push_back(LPS25H_PRESS_OUT_L);
    resultRegisters.push_back(LPS25H_TEMP_OUT_L);
...
int LPS25H::readRegister(int registerAddress, int size, byte_t &data) {
    ...
    return _hal->read(LPS25H_I2C_ADDR0, registerAddress, size, &data);
}
...
int LPS25H::writeRegister(int registerAddress, int size, byte_t &data) {
    ...
    return _hal->write(LPS25H_I2C_ADDR0, registerAddress, size, &data);
}
